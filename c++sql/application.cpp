...
int main(void)
{
    ...
    /* Used Variables */
    string str;
    string user = "dbs311_223naa15";
    string pass = "26495246";
...
void displayOrderStatus(Connection* conn, int orderId, int customerId) {
    Statement* stmt = conn->createStatement("BEGIN customer_order(:1, :2); END;"); //call customer_order()

    stmt->setInt(1, customerId);
    stmt->setInt(2, orderId);
    stmt->executeUpdate();

    //customerId = stmt->getInt(1); //don't need this, not used
    ...
    if (orderId) {
        ...
        stmt->registerOutParam(2, Type::OCCISTRING, sizeof(orderStatus));
        stmt->executeUpdate();
    ...
    else { //!orderId, orderId == 0
        cout << "OrderID is not valid." << endl;
    }

    conn->commit(); //add to database
    conn->terminateStatement(stmt); //close the statement object
}
...
void cancelOrder(Connection* conn, int orderId, int customerId) {
    ...
    if (orderId) {
        Statement* stmt = conn->createStatement("BEGIN cancel_order(:1, :2); END; "); //call cancel_order()
        int status;
        stmt->setInt(1, orderId);
        ...
        case 3:
            ...
            break;
        default:
            cout << "Error: cancel order failed" << endl;
        }
    }
    else { //!orderId, orderId == 0
        cout << "OrderID is not valid." << endl; 
